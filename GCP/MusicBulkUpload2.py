...
artists_path = sorted(os.listdir('./ROOTB/'))
artists = []
for artist_path in artists_path:
    artist = {}
    albums = []

    if os.path.isdir(f'./ROOTB/{artist_path}'):
        for artist_file in os.listdir(f'./ROOTB/{artist_path}/'):
            artist_file = artist_file.lower()
            if artist_file.endswith(".txt"):
                for line in open(f'./ROOTB/{artist_path}/{artist_file}', 'r').readlines():
                    artist[line.strip()[:line.strip().index(':')]] = line.strip()[
                        line.strip().index(':') + 2:]
            if artist_file.endswith(".jpg"):
                artist['artist_profileImage'] = str(
                    os.path.abspath(f'./ROOTB/{artist_path}/{artist_file}'))
        ...
        albums_path = sorted(os.listdir(f'./ROOTB/{artist_path}/'))
        for album_path in albums_path:
            ...
            if album_path == 'Singles':
                tracks_path = sorted(os.listdir(
                    f'./ROOTB/{artist_path}/Singles/'))
                for track_path in tracks_path:
                    track = {}
                    if os.path.isdir(f'./ROOTB/{artist_path}/Singles/{track_path}'):
                        for strack_file in os.listdir(f'./ROOTB/{artist_path}/Singles/{track_path}'):
                            strack_file = strack_file.lower()
                            if strack_file.endswith(".txt"):
                                if strack_file == "lyrics.txt":
                                    with open(f'./ROOTB/{artist_path}/Singles/{track_path}/{strack_file}') as lyrics:
                                    ...
                                    for line in open(f'./ROOTB/{artist_path}/Singles/{track_path}/{strack_file}', 'r').readlines():
                                        track[line.strip()[:line.strip().index(':')]] = line.strip()[
                                            line.strip().index(':') + 2:]

                            if strack_file.endswith(".jpg"):
                                track['track_coverImage'] = str(os.path.abspath(
                                    f'./ROOTB/{artist_path}/Singles/{track_path}/{strack_file}'))
            ...
            if os.path.isdir(f'./ROOTB/{artist_path}/{album_path}'):
                for album_file in os.listdir(f'./ROOTB/{artist_path}/{album_path}'):
                    album_file = album_file.lower()
                    if album_file.endswith(".txt"):
                        for line in open(f'./ROOTB/{artist_path}/{album_path}/{album_file}', 'r').readlines():
                            album[line.strip()[:line.strip().index(':')]] = line.strip()[
                                line.strip().index(':') + 2:]
                    if album_file.endswith(".jpg"):
                        album['album_coverImage'] = str(os.path.abspath(
                            f'./ROOTB/{artist_path}/{album_path}/{album_file}'))
                ...
                tracks_path = sorted(os.listdir(
                    f'./ROOTB/{artist_path}/{album_path}/'))
                for track_path in tracks_path:
                    track = {}
                    if os.path.isdir(f'./ROOTB/{artist_path}/{album_path}/{track_path}'):
                        for track_file in os.listdir(f'./ROOTB/{artist_path}/{album_path}/{track_path}'):
                            track_file = track_file.lower()
                            if track_file.endswith(".txt"):
                                if track_file == "lyrics.txt":
                                    with open(f'./ROOTB/{artist_path}/{album_path}/{track_path}/{track_file}') as lyrics:
                                        lines = lyrics.readlines()
                                        lyrics = ""
                                        for line in lines:
                                            lyrics = lyrics + line
                                    ...
                                    for line in open(f'./ROOTB/{artist_path}/{album_path}/{track_path}/{track_file}', 'r').readlines():
                                        track[line.strip()[:line.strip().index(':')]] = line.strip()[
                                            line.strip().index(':') + 2:]

                            if track_file.endswith(".jpg"):
                                track['track_coverImage'] = str(os.path.abspath(
                                    f'./ROOTB/{artist_path}/{album_path}/{track_path}/{track_file}'))
